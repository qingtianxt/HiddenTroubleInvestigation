...
<body>
	<div class="page-container">
		<form class="form form-horizontal" id="form-article-add"
			action="${pageContext.request.contextPath }/standardInvestigation_update.action"
			method="post" target="_parent">
			<input type="hidden" name="standardInvestigation_id"
				value="${standardInvestigation.standardInvestigation_id }">
			<input type="hidden" name="create_date"
				value="${standardInvestigation.create_date }">
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2"><span
					class="c-red">*</span>主题：</label>
				<div class="formControls col-xs-3 col-sm-4">
					<input type="text" class="input-text" placeholder="" id=""
						name="content" value="${standardInvestigation.content }">
				</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-3 col-sm-4">
					<span class="select-box"> <select name="repairName"
						id="repair" class="select">
							<option value="">请选择</option>
					</select>
					</span>
				</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<textarea name="info" cols="" rows="" class="textarea"
						...
						nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)"> ${standardInvestigation.info }</textarea>
											...
											function(data) {
												if (data != null
													...
													for ( var type in data) {
														if (data[type].id == ${place.parent_id}) {
							...
							.post(
									...
									function(data) {
										if (data != null
											...
											for ( var type in data) {
												if(data[type].user_id==${standardInvestigation.user.user_id}){
													content += "<option value='"+data[type].trueName+"' selected='selected'>"
													+ data[type].trueName
													+ "</option>";
												}else{
							...
							.post(
									...
									function(data) {
										if (data != null
											...
											var content = "";
											for ( var type in data) {
												if(data[type].user_id==${standardInvestigation.repair.user_id}){
													content += "<option value='"+data[type].trueName+"' selected='selected'>"
													+ data[type].trueName
													+ "</option>";
												}else{
